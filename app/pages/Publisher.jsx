...
import { totalPublisherFetched } from '../Redux/slice/publicDataSlice/totalPublisherSlice';
...
const Publisher = () => {
    ...
    const { publisherData, loading, error } = useSelector((state) => state.allPublisherData);
    const dispatch = useDispatch();
    const cardLoading = 10;


    useEffect(() => {
        dispatch(totalPublisherFetched())
    }, [])
                    ...
                    publisherData.length > 0 ? (
                        publisherData.map(publisher => (
                            <div className="border-2 border-[#bbb] hover:border-[#003A5A] hover:duration-300 flex flex-col justify-center items-center gap-5 p-5">
                                <img className="w-40 h-40 border rounded-full" src={publisher.publisher_image} alt="" />
                                <p className="text-xl">{publisher.publisher_name}</p>
